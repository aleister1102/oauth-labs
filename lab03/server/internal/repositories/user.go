...
func (u *UserRepository) Get(ctx context.Context, id string) (*dto.User, error) {
	stmt := `SELECT id, avatar_url, username, password, firstname, lastname, email, extra FROM users WHERE id = ?`
	row := u.db.QueryRowContext(ctx, stmt, id)
	var user dto.User
	err := row.Scan(&user.ID, &user.AvatarURL, &user.Username, &user.Password, &user.Firstname, &user.Lastname, &user.Email, &user.Extra)
	if err != nil {
		return nil, err
	}
	return &user, nil
...
func (u *UserRepository) GetByUsername(ctx context.Context, username string) (*dto.User, error) {
	stmt := `SELECT id, avatar_url, username, password, firstname, lastname, email, extra FROM users WHERE username = ?`
...
func (u *UserRepository) Create(ctx context.Context, username string, password string) error {
	cfg := config.Get()
	defaultAvatarURL := fmt.Sprintf("%s/static/img/avatar.png", cfg.GetString("oauth.issuer"))
	stmt := `INSERT INTO users(id, avatar_url, username, password) VALUES(?, ?, ?, ?)`
	_, err := u.db.ExecContext(ctx, stmt, uuid.NewString(), defaultAvatarURL, username, password)
	return err
}
...
func (u *UserRepository) Update(ctx context.Context, request *dto.UpdateProfile) error {
	...
	if request.AvatarURL != "" {
		_, _ = tx.Exec(`UPDATE users SET avatar_url = ? WHERE id = ?`, request.AvatarURL, request.UserID)
	}
