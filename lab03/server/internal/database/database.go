...
func Migrate(db *sql.DB) error {
	...
	avatarURL := fmt.Sprintf("%s/static/img/oauth_logo.png", cfg.GetString("oauth.issuer"))
	_, err = tx.Exec(`UPDATE users SET password = ?, avatar_url = ? WHERE username = 'admin'`, hash, avatarURL)
	if err != nil {
		log.Println(err.Error())
	}
