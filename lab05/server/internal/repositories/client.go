...
import (
	...
	"github.com/cyllective/oauth-labs/lab05/server/internal/entities"
)
...
func (r *ClientRepository) GetMany(ctx context.Context, ids ...string) ([]*entities.Client, error) {
...
func (r *ClientRepository) GetAll(ctx context.Context) ([]*entities.Client, error) {
	rows, err := r.db.QueryContext(ctx, "SELECT id, config FROM clients")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
