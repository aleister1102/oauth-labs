...
func (oa *OAuthService) Authorize(c *gin.Context, request *dto.AuthorizeRequest) (*AuthorizationCodeResponse, error) {
	...
	// redirect_uri validation - must be allowed by the client configuration
	{
...
func validateAuthorizeRedirectURI(uri string, allowedURIs []string) (*redirecturi.RedirectURI, *oalib.VerboseError) {
	...
	if uri != "" && !slices.Contains(allowedURIs, uri) {
		return nil, &oalib.VerboseError{
			Err:         "invalid_redirect_uri",
			Description: "request included a redirect_uri that is not allowed.",
		}
	}
	if uri == "" && len(allowedURIs) == 1 {
		uri = allowedURIs[0]
...
func (oa *OAuthService) handleAuthorizationCodeGrant(ctx context.Context, request *dto.TokenRequest, client *dto.Client) (*oalib.TokenResponse, *oalib.VerboseError) {
	...
	// If the code was created with a redirect_uri present, verify it matches
	// with the client's requested redirect_uri.
	if code.RedirectURI != "" && code.RedirectURI != request.RedirectURI {
		return nil, &oalib.VerboseError{
			Err:         "invalid_grant",
			Description: "client did not include the correct redirect_uri.",
		}
	}
	...
	if code.CodeChallenge != "" {
		if request.CodeVerifier == "" {
			return nil, &oalib.VerboseError{
				Err:         "invalid_grant",
				Description: "client did not include code_verifier.",
			}
		}
		ok, err := pkce.Verify(code.CodeChallengeMethod, code.CodeChallenge, request.CodeVerifier)
		if err != nil || !ok {
			return nil, &oalib.VerboseError{
				Err:         "invalid_grant",
				Description: "client code_verifier mismatch.",
			}
		}
	}
