...
import (
	...
	"github.com/cyllective/oauth-labs/lab01/client/internal/assets"
	"github.com/cyllective/oauth-labs/lab01/client/internal/client"
	"github.com/cyllective/oauth-labs/lab01/client/internal/config"
	"github.com/cyllective/oauth-labs/lab01/client/internal/constants"
	"github.com/cyllective/oauth-labs/lab01/client/internal/controllers"
	"github.com/cyllective/oauth-labs/lab01/client/internal/database"
	"github.com/cyllective/oauth-labs/lab01/client/internal/middlewares"
...
func configureRoutes(r *gin.Engine) error {
	...
	r.GET("/profile", middlewares.NoCache(), middlewares.TokensRequired(tokenService), func(c *gin.Context) {
		...
		clnt := client.NewAPIClient(ctx, tokens)
		...
		user, err := userService.Get(ctx, profile.Email)
		if err != nil {
			session.Delete(s)
			c.Redirect(http.StatusFound, "/")
			return
	...
	{
		c := controllers.NewOAuthController(oauthConfig, tokenService, userService)
