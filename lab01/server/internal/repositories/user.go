...
func (u *UserRepository) Get(ctx context.Context, id string) (*dto.User, error) {
	stmt := `SELECT id, username, password, firstname, lastname, email FROM users WHERE id = ?`
...
func (u *UserRepository) GetByUsername(ctx context.Context, username string) (*dto.User, error) {
	stmt := `SELECT id, username, password, firstname, lastname, email FROM users WHERE username = ?`
