...
func Configure() {
	dockerLabDir := filepath.Join(constants.DockerDir, "lab02")
...
client:
  ...
  scopes:
    - 'read:profile'
  uri: 'https://client-02.oauth.labs'
  logo_uri: 'https://client-02.oauth.labs/static/img/logo.png'
  redirect_uri: 'https://client-02.oauth.labs/callback'

authorization_server:
  issuer: 'https://server-02.oauth.labs'
  authorize_uri: 'https://server-02.oauth.labs/oauth/authorize'
  token_uri: 'http://server-02:3000/oauth/token'
  jwk_uri: 'http://server-02:3000/.well-known/jwks.json'
  revocation_uri: 'http://server-02:3000/oauth/revoke'
  register_uri: 'http://server-02:3000/oauth/register'
  registration_secret: '{{ .registration_secret }}'

resource_server:
  base_url: 'http://server-02:3000'

cookie:
  name: 'client-02'
  secret: '{{ .cookie_secret }}'
  domain: 'client-02.oauth.labs'
  path: '/'
  max_age: 80400
  http_only: true
  secure: true
