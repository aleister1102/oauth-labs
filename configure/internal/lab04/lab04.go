...
func Configure() {
	dockerLabDir := filepath.Join(constants.DockerDir, "lab04")
...
client:
  ...
  scopes:
    - 'read:profile'
  uri: 'https://client-04.oauth.labs'
  logo_uri: 'https://client-04.oauth.labs/static/img/logo.png'
  redirect_uri: 'https://client-04.oauth.labs/callback'

authorization_server:
  issuer: 'https://server-04.oauth.labs'
  authorize_uri: 'https://server-04.oauth.labs/oauth/authorize'
  token_uri: 'https://server-04.oauth.labs/oauth/token'
  jwk_uri: 'http://server-04:3000/.well-known/jwks.json'
  revocation_uri: 'http://server-04:3000/oauth/revoke'
  register_uri: 'http://server-04:3000/oauth/register'
  registration_secret: '{{ .registration_secret }}'

resource_server:
  base_url: 'http://server-04:3000'

cookie:
  name: 'client-04'
  secret: '{{ .cookie_secret }}'
  domain: 'client-04.oauth.labs'
  path: '/'
  max_age: 80400
  http_only: true
  secure: true
